...
void placeAntinodes() {
    ...
    for (int i = 0; i < numNotDots; i++) {
        for (int j = 0; j < numNotDots; j++) {
            if (i == j) {
                ...
                // Repeatedly looping the place the 'harmonic' antinodes
                for (int k = 1; k < params.linecount + 1; k++) {
                    // Checking if the proposed antinode is within bounds
                    if (Antennas[j].m + k * mchange >= params.linecount ||
                        Antennas[j].m + k * mchange < 0) {
                        break;
                    } else if (Antennas[j].n + k * nchange >= width ||
                               Antennas[j].n + k * nchange < 0) {
                        break;
                    } else if (Collection[Antennas[j].m + k * mchange]
                                         [Antennas[j].n + k * nchange] == '#') {
                        continue;
                    }
                    numAntinodes++;
                    printf("Antinode %d: %c (%d, %d)\n", numAntinodes,
                           Antennas[j].name, Antennas[j].m + k * mchange,
                           Antennas[j].n + k * nchange);

                    Collection[Antennas[j].m + k * mchange]
                              [Antennas[j].n + k * nchange] = '#';
                }
            }
        }
    ...
    for (int i = 0; i < params.linecount; i++) {
        for (int j = 0; j < width; j++) {
            if (Collection[i][j] != '#' && Collection[i][j] != '.') {
                numAntinodes++;
                Collection[i][j] = '#';
            }
        }
